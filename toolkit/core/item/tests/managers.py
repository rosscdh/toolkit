...
class ItemManagerTest(BaseScenarios, TestCase):
    ...
    def test_my_requests(self):
        # item, revisions: 0
        item1 = mommy.make('item.Item')

        # item, requested, revisions: 0, user: customer
        item2 = mommy.make('item.Item', is_requested=True, responsible_party=self.user)

        # item, requested, revisions: 0, user: lawyer
        item3 = mommy.make('item.Item', is_requested=True, responsible_party=self.lawyer)

        # item, reviewer: user, revision: stale
        item5 = mommy.make('item.Item')
        ...
        self.assertEquals(my_requests[0], item2)
        self.assertEquals(my_requests[1], item7)
        self.assertEquals(my_requests[2], item11)
