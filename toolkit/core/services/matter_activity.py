...
class MatterActivityEventService(object):
    ...
    def added_matter_participant(self, matter, adding_user, added_user):
        if adding_user.pk != added_user.pk:
            override_message = u'%s added %s as a participant of %s' % (adding_user, added_user, matter)
            self._create_activity(actor=adding_user, verb=u'added participant', action_object=matter, override_message=override_message,
                                  user=added_user)
    ...
    def removed_matter_participant(self, matter, removing_user, removed_user):
        ...
        self._create_activity(actor=removing_user, verb=u'edited', action_object=matter, override_message=override_message,
                              user=removed_user)
    #
    # Item focused events
    #
    ...
    def item_changed_status(self, user, item, previous_status):
        ...
        self._create_activity(actor=user, verb=u'changed the status', action_object=item, item=item, override_message=override_message,
    ...
    def item_closed(self, user, item):
        ...
        self._create_activity(actor=user, verb=u'closed', action_object=item, item=item, override_message=override_message)

    def item_reopened(self, user, item):
        override_message = u'%s reopened %s' % (user, item)
        self._create_activity(actor=user, verb=u'reopened', action_object=item, item=item, override_message=override_message)

    def add_item_comment(self, user, item, comment):
        override_message = '%s commented on %s' % (user, item)
        self._create_activity(actor=user, verb=u'commented', action_object=item, override_message=override_message, comment=comment)
    ...
    def created_revision(self, user, item, revision):
        override_message = u'%s created a revision for %s' % (user, item)
        self._create_activity(actor=user, verb=u'created', action_object=revision, item=item, override_message=override_message,
    ...
    def deleted_revision(self, user, item, revision):
        ...
        self._create_activity(actor=user, verb=u'deleted', action_object=revision, item=item, override_message=override_message,
                              filename=revision.name, date_created=revision.date_created)
    ...
    def request_user_upload_revision(self, item, adding_user, added_user):
        ...
        self._create_activity(actor=adding_user, verb=u'provide a document', action_object=item, override_message=override_message,
                              user=added_user)
    ...
    def add_revision_comment(self, user, revision, comment):
        ...
        self._create_activity(actor=user, verb=u'added revision comment', action_object=revision, override_message=override_message,
    ...
    def delete_revision_comment(self, user, revision):
        ...
        self._create_activity(actor=user, verb=u'deleted revision comment', action_object=revision, override_message=override_message,
                              item=revision.item)
    ...
    def invite_user_as_reviewer(self, item, inviting_user, invited_user):
        if inviting_user.pk != invited_user:
            ...
            self._create_activity(actor=inviting_user, verb=u'invited reviewer', action_object=item, override_message=override_message,
                                  user=invited_user)
    ...
    def user_commented_on_revision(self, item, user, revision, comment):
        ...
        self._create_activity(actor=user, verb=u'commented on revision', action_object=revision, override_message=override_message,
                              item=item, filename=revision.name, version=revision.slug,
                              date_created=datetime.datetime.utcnow(),
                              comment=comment)
    ...
    def user_revision_review_complete(self, item, user, revision):
        ...
        self._create_activity(actor=user, verb=u'completed review', action_object=item, override_message=override_message,
                              revision=revision, filename=revision.name, version=revision.slug,
                              date_created=datetime.datetime.utcnow())
